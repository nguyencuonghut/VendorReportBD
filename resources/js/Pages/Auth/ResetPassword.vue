<script setup>
import FloatingConfigurator from '../../SakaiVue/components/FloatingConfigurator.vue';
...
const passwordStrength = computed(() => {
    ...
    if (score < 3) {
        return { score, message: t('auth.passwordWeak'), color: 'text-red-600' };
    } else if (score < 4) {
        return { score, message: t('auth.passwordMedium'), color: 'text-yellow-600' };
    } else {
        return { score, message: t('auth.passwordStrong'), color: 'text-green-600' };
    }
});
...
const handleResetPassword = () => {
    ...
    if (passwordStrength.value.score < 3) {
        ToastService.error(t('auth.passwordTooWeak'));
        return;
    }

    if (!passwordsMatch.value) {
        ToastService.error(t('auth.passwordsDoNotMatch'));
        return;
    }
            ...
            <div style="border-radius: 56px; padding: 0.3rem; background: linear-gradient(180deg, var(--primary-color) 10%, rgba(33, 150, 243, 0) 30%)">
                <div class="w-full bg-surface-0 dark:bg-surface-900 py-20 px-8 sm:px-20" style="border-radius: 53px">
                    <div class="text-center mb-8">
                        ...
                        <div class="text-surface-900 dark:text-surface-0 text-3xl font-medium mb-4">{{ t('auth.resetPasswordTitle') }}</div>
                        <span class="text-muted-color font-medium">{{ t('auth.resetPasswordSubtitle') }}</span>
                    </div>
                    ...
                    <form @submit.prevent="handleResetPassword">
                        ...
                        <div class="mb-4">
                            <label for="password1" class="block text-surface-900 dark:text-surface-0 font-medium text-xl mb-2">{{ t('auth.newPassword') }}</label>
                            <Password
                                id="password1"
                                v-model="password"
                            ...
                            <small v-if="password" :class="passwordStrength.color" class="block mt-1">{{ passwordStrength.message }}</small>
                        </div>

                        <div class="mb-6">
                            <label for="password_confirmation" class="block text-surface-900 dark:text-surface-0 font-medium text-xl mb-2">{{ t('auth.confirmPassword') }}</label>
                            <Password
                                id="password_confirmation"
                                v-model="passwordConfirmation"
                                :placeholder="t('auth.confirmPasswordPlaceholder')"
                                :toggleMask="true"
                                :class="{ 'p-invalid': hasError('password_confirmation') || (passwordConfirmation && !passwordsMatch) }"
                                class="w-full"
                                fluid
                                :feedback="false"
                                :disabled="processing"
                            ...
                            <small v-if="passwordConfirmation && !passwordsMatch" class="p-error block mt-1">{{ t('auth.passwordsDoNotMatch') }}</small>
                        </div>

                        <div class="flex flex-col gap-4">
                            <Button
                                type="submit"
                                :label="processing ? t('auth.resetting') : t('auth.resetPassword')"
                                class="w-full"
                                :loading="processing"
                                :disabled="processing || passwordStrength.score < 3 || !passwordsMatch"
                            />
